...
/**
 * \brief   Destructor. Turns leds off if initialized before.
 */
Leds::~Leds()
{
    Sleep();
...
bool Leds::Init()
{
    ...
    if (!mInitialized)
    {
        if (NUMBER_OF_NEOPIXELS > 0)
        {
            ...
            mStrip.fill(ConvertColor(LedColor::Off), 0, NUMBER_OF_NEOPIXELS);
...
/**
 * \brief   Indicate if Leds are initialized.
 * \returns True if Leds are initialized, else false.
 */
bool Leds::IsInit() const
{
    return mInitialized;
}

/**
 * \brief   Puts the Leds module in sleep mode.
 * \returns True if Leds module could be put in sleep mode, else false.
 */
bool Leds::Sleep()
{
    if (mInitialized)
